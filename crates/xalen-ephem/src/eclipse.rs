...
//!
//! Algorithms follow Meeus, *Astronomical Algorithms* (2nd ed.), chapters 54
//! (lunar eclipses) and 55 (solar eclipses), with latitude-threshold
//! classification from the *Explanatory Supplement to the Astronomical Almanac*.
//!
//! # Approach
//!
//! 1. Step through the date range in ~1-day increments.
//! 2. For each step, locate exact New Moon (solar) or Full Moon (lunar)
//!    using bisection on the Sun-Moon elongation function.
//! 3. Evaluate the Moon's ecliptic latitude at that moment.
//! 4. Classify by comparing |latitude| against empirical threshold cones.
...
// Solar eclipse latitude threshold (degrees, absolute value).
// Meeus Ch. 55 -- the maximum latitude at which *any* eclipse geometry is
// possible at New Moon.
const SOLAR_ECLIPSE_LIMIT: f64 = 1.58;

// The geocentric latitude limit below which a *central* eclipse (total or
// annular) can occur. Derived from Meeus Ch. 55: central eclipses require
// the shadow axis to pass close to Earth's center, corresponding to gamma
// values |gamma| < 0.9972. In the latitude-based approximation this
// translates to roughly |lat| < 0.9972 degrees.
const SOLAR_CENTRAL_LIMIT: f64 = 0.9972;
...
pub struct LunarEclipse {
    /// Julian Date (UT1) of maximum eclipse (exact Full Moon moment).
    pub jd_maximum: f64,
...
pub struct SolarEclipse {
    /// Julian Date (UT1) of maximum eclipse (exact New Moon moment).
    pub jd_maximum: f64,
    /// Eclipse classification (geocentric estimate).
...
pub fn find_solar_eclipses(almanac: &Almanac, jd_start: f64, jd_end: f64) -> Vec<SolarEclipse> {
    ...
    for jd_new in new_moons {
        ...
        if lat_deg.abs() < SOLAR_ECLIPSE_LIMIT {
            ...
            // Classification via the gamma-based approach (Meeus Ch. 55):
            //
            // A *central* eclipse (total or annular) occurs when the lunar
            // shadow axis passes close enough to Earth's center. In our
            // latitude-based approximation, this corresponds to:
            //   |latitude| < SOLAR_CENTRAL_LIMIT (~0.9972 degrees)
            //
            // Within central eclipses, the sub-type depends on whether the
            // Moon's apparent disk is larger or smaller than the Sun's:
            //   diameter_ratio >= 1.0 => Total (Moon covers Sun)
            //   diameter_ratio <  1.0 => Annular (ring of sunlight visible)
            //
            // If the latitude is larger than the central limit but still
            // within the eclipse limit, only a partial eclipse is visible.
            let eclipse_type = if separation_deg < SOLAR_CENTRAL_LIMIT {
            ...
            // Magnitude: fraction of the Sun's diameter obscured at maximum.
            //
            ...
            let magnitude = match eclipse_type {
                SolarEclipseType::Total | SolarEclipseType::Annular => diameter_ratio,
                SolarEclipseType::Partial => {
