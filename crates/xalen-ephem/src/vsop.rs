...
/// VSOP87-based analytical ephemeris provider (Tier 0, ~1 arcsecond accuracy).
...
impl EphemerisProvider for Vsop87Provider {
    fn heliocentric_ecliptic(
        ...
        if body == Body::Pluto {
            return crate::pluto::pluto_position(jd_tt);
        }
        let (x, y, z) = self.helio_rect(body, jd_tt.as_f64())?;
        let mut pos =
            xalen_coords::cartesian_to_ecliptic(&xalen_coords::CartesianPosition { x, y, z });
    ...
    fn geocentric_ecliptic(
        ...
        if body == Body::Moon {
            ...
            // Do NOT add IAU 2000B nutation here. Measured against JPL Horizons
            // apparent ObsEcLon, the truncated Meeus Ch.47 series as implemented
            // already tracks the apparent (true-equinox-of-date) longitude to
            // within its truncation bound: J2000 2.5", 2050 1.8". Adding delta_psi
            // separately DEGRADES those to ~12" (a double-count of nutation-sized
            // magnitude). The residual ~18" at some epochs is the 60-term series
            // truncation limit, not a missing-nutation error. Verified 2026-05-28.
        ...
        if body == Body::Pluto {
            // Meeus Ch.37 gives heliocentric ecliptic (J2000) — convert to geocentric
            // by subtracting Earth's heliocentric position, same pattern as other planets.
            let t = jd_tt.julian_centuries_from_j2000();
            ...
            let helio_0 = crate::pluto::pluto_position(jd_tt)?;
            let helio_j2000_0 = xalen_coords::EclipticPosition {
                longitude: helio_0.longitude - precession,
                latitude: helio_0.latitude,
                distance: helio_0.distance,
            };
            let cart_0 = xalen_coords::ecliptic_to_cartesian(&helio_j2000_0);
            let dx = cart_0.x - ex;
            let dy = cart_0.y - ey;
            let dz = cart_0.z - ez;
            ...
            // Step 2: re-evaluate Pluto at retarded time (t - tau)
            let jd_retarded = JdTT(jd_tt.as_f64() - tau_0);
            let helio_1 = crate::pluto::pluto_position(jd_retarded)?;
            // Strip precession computed at the ORIGINAL epoch (precession is observer-side)
            let helio_j2000_1 = xalen_coords::EclipticPosition {
                longitude: helio_1.longitude
                    - xalen_coords::general_precession_longitude(
                        jd_retarded.julian_centuries_from_j2000(),
                    ),
                latitude: helio_1.latitude,
                distance: helio_1.distance,
            };
            let cart_1 = xalen_coords::ecliptic_to_cartesian(&helio_j2000_1);

            // Iteration: recompute distance from retarded position, get refined tau
            let dx1 = cart_1.x - ex;
            let dy1 = cart_1.y - ey;
            let dz1 = cart_1.z - ez;
            ...
            let helio_final = crate::pluto::pluto_position(jd_final)?;
            let helio_j2000_final = xalen_coords::EclipticPosition {
                longitude: helio_final.longitude
                    - xalen_coords::general_precession_longitude(
                        jd_final.julian_centuries_from_j2000(),
                    ),
                latitude: helio_final.latitude,
                distance: helio_final.distance,
            };
            let cart_final = xalen_coords::ecliptic_to_cartesian(&helio_j2000_final);
            ...
            let mut pos = xalen_coords::cartesian_to_ecliptic(&geo_cart);
            pos.longitude += precession;
            let pos = apply_nutation(pos, t);
            let pos = aberration_correction(pos, jd_tt);
            return Ok(pos.normalize());
        }
        ...
        if body == Body::Chiron {
            // --- Light-time iteration (1 pass) for Chiron ---
            let (ex, ey, ez) = self.helio_rect(Body::Earth, jd_tt.as_f64())?;
            let t = jd_tt.julian_centuries_from_j2000();
            let precession = xalen_coords::general_precession_longitude(t);
...
pub(crate) fn aberration_correction(
    ...
    // Compute Sun's geometric longitude (equinox-of-date) from Earth's VSOP87 heliocentric position
