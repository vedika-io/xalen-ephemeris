...
//   Moon:    3" at J2000, 17" at 2024  (ELP2000-82, 60 longitude terms)
...
// After nutation fix: VSOP87 planets within 1.1", Moon within 3"
const JPL_J2000_REFS: &[JplRef] = &[
    JplRef { body: Body::Sun,     label: "Sun",     expected_deg: 280.3689, tolerance_deg: 0.001 },
    JplRef { body: Body::Moon,    label: "Moon",    expected_deg: 223.3238, tolerance_deg: 0.006 },
    JplRef { body: Body::Mercury, label: "Mercury", expected_deg: 271.8893, tolerance_deg: 0.001 },
    JplRef { body: Body::Venus,   label: "Venus",   expected_deg: 241.5658, tolerance_deg: 0.001 },
    JplRef { body: Body::Mars,    label: "Mars",    expected_deg: 327.9633, tolerance_deg: 0.001 },
    JplRef { body: Body::Jupiter, label: "Jupiter", expected_deg:  25.2531, tolerance_deg: 0.001 },
    JplRef { body: Body::Saturn,  label: "Saturn",  expected_deg:  40.3956, tolerance_deg: 0.001 },
    JplRef { body: Body::Uranus,  label: "Uranus",  expected_deg: 314.8092, tolerance_deg: 0.001 },
    JplRef { body: Body::Neptune, label: "Neptune", expected_deg: 303.1930, tolerance_deg: 0.001 },
];

// JPL Horizons DE440 — 1950-01-01 12:00 UT (50 years before J2000)
const JPL_1950_REFS: &[JplRef] = &[
    JplRef { body: Body::Sun,     label: "Sun",     expected_deg: 280.5144, tolerance_deg: 0.001 },
    JplRef { body: Body::Moon,    label: "Moon",    expected_deg:  67.5445, tolerance_deg: 0.008 },
    JplRef { body: Body::Jupiter, label: "Jupiter", expected_deg: 306.6180, tolerance_deg: 0.001 },
    JplRef { body: Body::Saturn,  label: "Saturn",  expected_deg: 169.4355, tolerance_deg: 0.001 },
];

// JPL Horizons DE440 — 2024-01-01 12:00 UT
const JPL_2024_REFS: &[JplRef] = &[
    JplRef { body: Body::Sun,     label: "Sun",     expected_deg: 280.5485, tolerance_deg: 0.001 },
    JplRef { body: Body::Moon,    label: "Moon",    expected_deg: 161.9070, tolerance_deg: 0.008 },
    JplRef { body: Body::Mars,    label: "Mars",    expected_deg: 267.6791, tolerance_deg: 0.001 },
    JplRef { body: Body::Jupiter, label: "Jupiter", expected_deg:  35.5844, tolerance_deg: 0.001 },
];

// JPL Horizons DE440 — 2050-01-01 12:00 UT (50 years after J2000)
const JPL_2050_REFS: &[JplRef] = &[
    JplRef { body: Body::Sun,     label: "Sun",     expected_deg: 281.2579, tolerance_deg: 0.001 },
    JplRef { body: Body::Moon,    label: "Moon",    expected_deg:  25.3810, tolerance_deg: 0.006 },
    JplRef { body: Body::Jupiter, label: "Jupiter", expected_deg: 121.6322, tolerance_deg: 0.001 },
    JplRef { body: Body::Saturn,  label: "Saturn",  expected_deg: 297.6321, tolerance_deg: 0.001 },
];
    ...
    for r in JPL_J2000_REFS {
        ...
        if delta > r.tolerance_deg {
            failures.push(format!(
                ...
                r.label, r.expected_deg, lon, delta, delta * 3600.0, r.tolerance_deg
            ));
        }
    }
    ...
    for r in JPL_1950_REFS {
        ...
        if delta > r.tolerance_deg {
            failures.push(format!(
                ...
                r.label, r.expected_deg, lon, delta, delta * 3600.0
            ));
        }
    }
    assert!(failures.is_empty(), "1950 failures:\n{}", failures.join("\n"));
}
...
fn jpl_planetary_longitudes_2024() {
    ...
    assert!(failures.is_empty(), "2024 failures:\n{}", failures.join("\n"));
}
...
fn jpl_planetary_longitudes_2050() {
    ...
    assert!(failures.is_empty(), "2050 failures:\n{}", failures.join("\n"));
}
...
fn jpl_sun_j2000() {
    let a = almanac();
    let lon = a.geocentric_longitude_deg(Body::Sun, JdUT1(JD_J2000)).unwrap();
    let delta = angle_delta(lon, 280.3689);
    assert!(delta < 0.001, "Sun at J2000: delta={delta:.4}° ({:.1}\")", delta * 3600.0);
}

#[test]
fn jpl_moon_j2000() {
    let a = almanac();
    let lon = a.geocentric_longitude_deg(Body::Moon, JdUT1(JD_J2000)).unwrap();
    let delta = angle_delta(lon, 223.3238);
    assert!(delta < 0.006, "Moon at J2000: delta={delta:.4}° ({:.1}\")", delta * 3600.0);
}

#[test]
fn jpl_mercury_j2000() {
    let a = almanac();
    let lon = a.geocentric_longitude_deg(Body::Mercury, JdUT1(JD_J2000)).unwrap();
    let delta = angle_delta(lon, 271.8893);
    assert!(delta < 0.001, "Mercury at J2000: delta={delta:.4}° ({:.1}\")", delta * 3600.0);
}

#[test]
fn jpl_venus_j2000() {
    let a = almanac();
    let lon = a.geocentric_longitude_deg(Body::Venus, JdUT1(JD_J2000)).unwrap();
    let delta = angle_delta(lon, 241.5658);
    assert!(delta < 0.001, "Venus at J2000: delta={delta:.4}° ({:.1}\")", delta * 3600.0);
}

#[test]
fn jpl_mars_j2000() {
    let a = almanac();
    let lon = a.geocentric_longitude_deg(Body::Mars, JdUT1(JD_J2000)).unwrap();
    let delta = angle_delta(lon, 327.9633);
    assert!(delta < 0.001, "Mars at J2000: delta={delta:.4}° ({:.1}\")", delta * 3600.0);
}

#[test]
fn jpl_jupiter_j2000() {
    let a = almanac();
    let lon = a.geocentric_longitude_deg(Body::Jupiter, JdUT1(JD_J2000)).unwrap();
    let delta = angle_delta(lon, 25.2531);
    assert!(delta < 0.001, "Jupiter at J2000: delta={delta:.4}° ({:.1}\")", delta * 3600.0);
}

#[test]
fn jpl_saturn_j2000() {
    let a = almanac();
    let lon = a.geocentric_longitude_deg(Body::Saturn, JdUT1(JD_J2000)).unwrap();
    let delta = angle_delta(lon, 40.3956);
    assert!(delta < 0.001, "Saturn at J2000: delta={delta:.4}° ({:.1}\")", delta * 3600.0);
}

#[test]
fn jpl_uranus_j2000() {
    let a = almanac();
    let lon = a.geocentric_longitude_deg(Body::Uranus, JdUT1(JD_J2000)).unwrap();
    let delta = angle_delta(lon, 314.8092);
    assert!(delta < 0.001, "Uranus at J2000: delta={delta:.4}° ({:.1}\")", delta * 3600.0);
}

#[test]
fn jpl_neptune_j2000() {
    let a = almanac();
    let lon = a.geocentric_longitude_deg(Body::Neptune, JdUT1(JD_J2000)).unwrap();
    let delta = angle_delta(lon, 303.1930);
    assert!(delta < 0.001, "Neptune at J2000: delta={delta:.4}° ({:.1}\")", delta * 3600.0);
}
...
fn house_cusps_pune_j2000() {
    ...
    assert!((angle_delta(asc, desc) - 180.0).abs() < 0.01, "ASC-DSC not opposite");

    for i in 0..12 {
        let c = h.cusp_deg(i);
        assert!(c >= 0.0 && c < 360.0 && !c.is_nan(), "Cusp {} invalid: {c}", i + 1);
    }
}
...
fn nutation_j2000() {
    ...
    assert!((dpsi - (-14.0)).abs() < 1.0, "dpsi at J2000: got {dpsi:.2}\"");
    assert!((deps - (-5.8)).abs() < 1.0, "deps at J2000: got {deps:.2}\"");
}
...
fn provider_tt_vs_almanac_ut1() {
    ...
    let ut1 = a.geocentric_longitude_deg(Body::Sun, JdUT1(JD_J2000)).unwrap();
    let tt = p.geocentric_ecliptic(Body::Sun, JdTT(JD_J2000)).unwrap()
        .longitude.to_degrees().rem_euclid(360.0);

    let diff = angle_delta(ut1, tt);
    assert!(diff < 0.01, "UT1 vs TT gap: {diff:.6}° (should be < 0.01° from ~64s delta-T)");
}
...
fn all_vsop87_bodies_compute() {
    let p = Vsop87Provider::new();
    let jd = JdTT::J2000;
    for body in [Body::Sun, Body::Moon, Body::Mercury, Body::Venus, Body::Mars,
